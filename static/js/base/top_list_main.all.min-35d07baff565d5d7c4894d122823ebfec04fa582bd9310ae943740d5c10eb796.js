...
(function () {
  ...
  $document.ready(function () {
    try {
      ...
      AdRepeater.repeat(
        adm.repo.where({page: 'toplist_list', name: /card_list.*/}),
        $(".top_list_item"),
        0,
    ...
    var page = 1;
    var service_call_count = 1;
    var template = $dom.top_list_item_template.html();
    var keyword;
    ...
    $dom.btn_more.on("click", function () {
      var start_index = REQUEST_COUNT * page;
      call_top_list_by_keyword(keyword, start_index, REQUEST_COUNT);
    });

    $dom.tag_item.on("click", function () {
      var $this = $(this);

      keyword = $this.data("keyword");
      page = 0;

      $dom.tag_item.removeClass(SELECTED_CLASS);
      $this.addClass(SELECTED_CLASS);
      $dom.btn_more.hide();

      $(".ad_area").remove();
      $(".top_list_item").remove();
      call_top_list_by_keyword(keyword, 0, REQUEST_COUNT);
    });

    function call_top_list_by_keyword(keyword, start_index, request_count) {
      if (keyword) {
        handleToplistData(window.mp20.utils.HttpApi.getTopListByKeyword(keyword, start_index, request_count), start_index)
      } else {
        handleToplistData(window.mp20.utils.HttpApi.getTopList(start_index, request_count), start_index)
      }
    }

    function handleToplistData(promise, start_index) {
      page++;

      promise.then(function (top_Lists) {
        if (top_Lists.length < 20) {
          $dom.btn_more.hide();
        } else {
          $dom.btn_more.show();
        }
        ...
        var more = _.map(top_Lists, function (top_list, index) {
          var $top_list_item = $(template({
            link_key: top_list.link_key,
            title: top_list.title,
            position: (start_index + index),
            description: top_list.adlib_message || top_list.description,
            picture_url: get_full_picture_url_by_akamai(top_list.picture_url, 585, 585)
          }));

          $dom.list_type_ls.append($top_list_item);
        ...
        if (start_index == 0) {
          if (adm) {
            AdRepeater.repeat(
              ...
              0,
              function (inventory, item, index) {
                adm.area_renderer.render(inventory, item, AdPlacer.after, service_call_count + "_" + index, 'card_list');
              }
            );
            service_call_count++;
            adm.publish();
        ...
        Ellipsis({
          "class": ".top_list_item .title",
          lines: 3
        });

        Ellipsis({
          "class": ".top_list_item .desc",
          lines: 2
        });
      });
    }
